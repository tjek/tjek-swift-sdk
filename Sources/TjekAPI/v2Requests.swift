...
extension TjekAPI {
    ...
    public func send<ResponseType>(v2 request: APIRequest<ResponseType>, completion: @escaping (Result<ResponseType, APIError>) -> Void) {
        v2.send(request, completion: completion)
    }
    
    public func send<ResponseType>(_ v2Req: APIv2Request<ResponseType>, completion: @escaping (Result<ResponseType, APIError>) -> Void) {
        send(v2: v2Req.request, completion: completion)
    }
}
