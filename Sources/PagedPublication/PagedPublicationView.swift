...
open class PagedPublicationView: UIView {
    ...
    open func reload(with loader: PagedPublicationLoaderProtocol, jumpTo pageIndex: Int = 0) {
        DispatchQueue.main.async { [weak self] in
            ...
            s.configureBasics(backgroundColor: loader.preloadedPublication?.bgColor,
    ...
    class PageNumberLabel: UILabel {
        ...
        // MARK: Edge Insets
        var labelEdgeInsets: UIEdgeInsets = UIEdgeInsets(top: 4, left: 22, bottom: 4, right: 22)
    ...
    public func didEndZooming(pages pageIndexes: IndexSet, zoomScale: CGFloat, in verso: VersoView) {
        ...
        if zoomScale > 1 {
            for pageIndex in pageIndexes {
                if let pageView = verso.getPageViewIfLoaded(pageIndex) as? PagedPublicationPageView, pageView.zoomImageLoadState == .notLoaded,
