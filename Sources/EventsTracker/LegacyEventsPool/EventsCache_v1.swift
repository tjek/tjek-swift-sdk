...
class EventsCache_v1: PoolCache_v1Protocol {
    ...
    lazy fileprivate var allObjects: [SerializedV1PoolObject] = {
        let start = Date().timeIntervalSinceReferenceDate
        
        var objs: [SerializedV1PoolObject] = []
    ...
    func remove(poolIds: [String]) {
        ...
        cacheInMemoryQueue.async {
            ...
            var trimmedObjs: [SerializedV1PoolObject] = []
    ...
    fileprivate func retreiveFromDisk() -> [SerializedV1PoolObject]? {
        ...
        if let objDicts = NSArray(contentsOfFile: path) {
            ...
            var serializedObjs: [SerializedV1PoolObject] = []
    ...
    fileprivate func saveToDisk(objects: [SerializedV1PoolObject]) {
        guard let path = diskCachePath else { return }
        
        // map [(poolId, jsonData)] -> [[String:AnyObject]]
        let objDicts: [[String: AnyObject]] = objects.map { (object: SerializedV1PoolObject) in
